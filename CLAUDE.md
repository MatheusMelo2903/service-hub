...
Em 2026-07-07 public/previsao-modulo.html (módulo de Previsão Orçamentária, Modo A "planilha pronta/final") recebeu 3 gaps corrigidos + 1 hardening: Gap A (regex da linha de TAXA reconhece todos os rótulos usados pelos condomínios), Gap B camada 1 (total a ratear ancorado na linha de TAXA, não somado das categorias), Gap C (cor do texto na prévia da planilha) e hardening da Guarda 2 (bloqueia taxa base zero e linha de TAXA duplicada divergente). 3897 linhas (contagem via `wc -l`; MD5 não registrado nesta entrada porque o hash colide com o padrão do hook anti-token do pre-commit). Validado por harness Node em scripts/previsao-harness/ contra 5 planilhas reais (5/5 batendo reajuste e mensal a ratear). Commits: a427f10, 1a5d56c, b1a6496, ff98d67, 7d7cb7e. Doc completa em tarefas/concluidas/previsao-modo-a-gap-abc.md. Pendências abertas: Gap B camada 2 (decisão do Matheus sobre consumo no panorama), validação visual no dev (aguarda autorização de push) e remoção de skills-server/Previsao_Naturale_2026.xlsx (planilha real de cliente no git).
