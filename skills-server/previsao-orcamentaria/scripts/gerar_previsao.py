...
try:
    ...
    sys.stderr.write("ERRO: openpyxl não instalado. Rode: pip3 install openpyxl --break-system-packages\n")
    sys.exit(1)
...
def carregar_planilha(caminho):
    """Lê a planilha de previsão e retorna um dict com todos os dados normalizados.
    ...
    Retorna:
        {
            ...
            "previsto": float,
            "fundo_reserva": float,
            "fundo_pct": float,      # 0.05 = 5%
            "apartamentos": int,
            "coberturas": int,
            "fator_cobertura": float,
            "unid_equiv": float,
            "categorias": [
                {
                    "nome": str,
                    "base": float,
                    "previsto": float,
                    "reajuste_pct": float,   # 0.07 = 7%
                    "tem_reajuste": bool,
                    "itens": [
                        {"nome": str, "base": float, "previsto": float, "reajuste_pct": float},
                        ...
                    ],
                },
                ...
            ],
            "previsao_mensal": [float] * 12,
            "meses": ["Jan/26", ...],
        }
    """
    wb = openpyxl.load_workbook(caminho, data_only=True)
    if "Previsao Anual" not in wb.sheetnames:
        raise ValueError("Planilha sem aba 'Previsao Anual'.")
    if "Resumo Assembleia" not in wb.sheetnames:
        raise ValueError("Planilha sem aba 'Resumo Assembleia'.")
    if "Previsao Mensal" not in wb.sheetnames:
        raise ValueError("Planilha sem aba 'Previsao Mensal'.")

    dados = {"condominio": None}

    # ---- Resumo Assembleia: condomínio, totais, unidades ----
    ws = wb["Resumo Assembleia"]
    for row in ws.iter_rows(values_only=True):
        for cell in row:
            if isinstance(cell, str) and "Condomínio" in cell and "Base" in cell:
                # Linha tipo "Condomínio Praia Dourada — Base: Realizado Superlógica"
                nome = cell.split("—")[0].strip()
                if nome.lower().startswith("condomínio"):
                    nome = nome.replace("Condomínio", "").strip()
                dados["condominio"] = "Condomínio " + nome if nome else None
                break

    # Despesa total, fundo, unidades
    dados["base_anual"] = None
    dados["previsto"] = None
    dados["fundo_reserva"] = None
    dados["fundo_pct"] = 0.05
    dados["apartamentos"] = None
    dados["coberturas"] = None
    dados["fator_cobertura"] = 1.5
    ...
    for i, row in enumerate(rows):
        for j, cell in enumerate(row):
            if not isinstance(cell, str):
                continue
            label = cell.strip()
            # Procura o valor: pode estar na mesma linha ou na linha de baixo
            val = None
            if i + 1 < len(rows) and len(rows[i + 1]) > j:
                val_below = rows[i + 1][j]
                if isinstance(val_below, (int, float)):
                    val = val_below
            if val is None and len(row) > j + 1 and isinstance(row[j + 1], (int, float)):
                val = row[j + 1]

            if val is None:
                continue

            if "Despesa Total Anual Prevista" in label:
                dados["previsto"] = float(val)
            elif "Fundo de Reserva" in label:
                dados["fundo_reserva"] = float(val)
                # Extrai % se vier no rótulo, tipo "Fundo de Reserva (5%)"
                if "%" in label:
                    try:
                        pct_str = label.split("(")[1].split("%")[0]
                        dados["fundo_pct"] = float(pct_str.replace(",", ".")) / 100
                    except Exception:
                        pass
            elif label == "Apartamentos:":
                dados["apartamentos"] = int(val)
            elif label == "Coberturas:":
                dados["coberturas"] = int(val)
            elif "Fator da Cobertura" in label:
                dados["fator_cobertura"] = float(val)

    # ---- Previsao Anual: categorias e itens com base e previsto ----
    ws = wb["Previsao Anual"]
    rows = list(ws.iter_rows(values_only=True))

    categorias = []
    cat_atual = None
    # Header: Item | Base Anual | Reajuste % | Previsão 2026
    for row in rows:
        # Item está na col B (idx 1), Base C (2), Reajuste D (3), Previsao E (4)
        if len(row) < 5:
            continue
        item, base, reaj, prev = row[1], row[2], row[3], row[4]
        ...
        # Linha "TOTAL GERAL" — finaliza
        if "TOTAL GERAL" in item.upper():
            if isinstance(base, (int, float)):
                dados["base_anual"] = float(base)
            if isinstance(prev, (int, float)):
                dados["previsto"] = float(prev)
            continue

        # Cabeçalho de categoria: tem nome em col B mas Base/Previsto vazios na linha
        if base is None and prev is None and reaj is None:
            # Pode ser cabeçalho ou linha decorativa
            if item.startswith("Total"):
                continue
            # SÓ aceita categorias da lista canônica — qualquer outra coisa
            # (títulos de aba, instruções, decorativas) é ignorada
            if item in ORDEM_CATEGORIAS_PADRAO:
                cat_atual = {
                    "nome": item,
                    "base": 0.0,
                    "previsto": 0.0,
        ...
        # Linha de total da categoria (vem com "  Total ...")
        if item.lstrip().startswith("Total"):
            if cat_atual is not None:
                if isinstance(base, (int, float)):
                    cat_atual["base"] = float(base)
                if isinstance(prev, (int, float)):
                    cat_atual["previsto"] = float(prev)
            continue

        # Linha de item: tem base e previsto numéricos
        if cat_atual is not None and isinstance(base, (int, float)) and isinstance(prev, (int, float)):
            reaj_pct = float(reaj) if isinstance(reaj, (int, float)) else 0.0
            cat_atual["itens"].append({
                "nome": item,
                "base": float(base),
                "previsto": float(prev),
                "reajuste_pct": reaj_pct,
            })

    # Calcular reajuste % da categoria (média ponderada pelos itens)
    for cat in categorias:
        if cat["base"] > 0:
            cat["reajuste_pct"] = (cat["previsto"] - cat["base"]) / cat["base"]
            # Arredonda para 4 casas para evitar 0.07000000000006
            cat["reajuste_pct"] = round(cat["reajuste_pct"], 4)
        # Conta como reajuste se >= 0.01% (filtra ruído de ponto flutuante)
        cat["tem_reajuste"] = abs(cat["reajuste_pct"]) >= 0.0001

    dados["categorias"] = categorias

    # Validação: soma de categorias deve bater com previsto
    soma_cats = sum(c["previsto"] for c in categorias)
    if dados["previsto"] and abs(soma_cats - dados["previsto"]) > 1.0:
        sys.stderr.write(
            f"AVISO: soma categorias R$ {soma_cats:.2f} != previsto R$ {dados['previsto']:.2f}\n"
        )

    # ---- Previsao Mensal: total mensal ----
    ws = wb["Previsao Mensal"]
    rows = list(ws.iter_rows(values_only=True))
    meses = ["Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"]
    previsao_mensal = [0.0] * 12
    for row in rows:
        if len(row) < 14:
            continue
        item = row[1]
        ...
        if "TOTAL GERAL" in item.upper():
            for k in range(12):
                ...
                if isinstance(v, (int, float)):
                    previsao_mensal[k] = float(v)
            break
    dados["previsao_mensal"] = previsao_mensal
    dados["meses"] = meses

    # Unidades equivalentes
    apt = dados["apartamentos"] or 0
    cob = dados["coberturas"] or 0
    fator = dados["fator_cobertura"] or 1.5
    dados["unid_equiv"] = apt + cob * fator

    if not dados.get("condominio"):
        dados["condominio"] = "Condomínio"

    return dados


def _provavel_categoria(nome):
    """DEPRECATED: substituído por allowlist ORDEM_CATEGORIAS_PADRAO.

    Mantido apenas como fallback. Se algum dia a planilha trouxer uma categoria
    nova (fora da lista canônica), basta adicioná-la em ORDEM_CATEGORIAS_PADRAO.
    """
    return False
...
# ========================================================================
def main():
    parser = argparse.ArgumentParser(description="Gerador de Previsão Orçamentária")
    parser.add_argument("planilha", help="Caminho para a planilha XLSX de previsão")
    parser.add_argument("--condominio", help="Sobrescreve o nome do condomínio")
    parser.add_argument("--output", help="Caminho do PPTX de saída", default=None)
    ...
    print(f"Base anual: {fmt_brl(dados['base_anual'])}")
    print(f"Previsto:   {fmt_brl(dados['previsto'])}")
    print(f"Fundo:      {fmt_brl(dados['fundo_reserva'] or 0)} ({dados['fundo_pct']*100:.0f}%)")
    print(f"Aptos: {dados['apartamentos']} | Coberturas: {dados['coberturas']} | Fator: {dados['fator_cobertura']} | UE: {dados['unid_equiv']}")
    print(f"Taxa apto: {fmt_brl(taxas['taxa_apto_antes'])} → {fmt_brl(taxas['taxa_apto_depois'])} ({fmt_pct_signed(taxas['impacto_pct'])})")
    print(f"Categorias com reajuste: {len(dados['categorias']) - len([c for c in dados['categorias'] if not c['tem_reajuste']])}")
    for c in dados["categorias"]:
        if c["tem_reajuste"]:
            print(f"  • {c['nome']}: {fmt_pct_signed(c['reajuste_pct'])} ({fmt_brl(c['base'])} → {fmt_brl(c['previsto'])})")

    # Localizar logo
    skill_dirs = [
        "/mnt/skills/user/previsao-orcamentaria",
        "/mnt/skills/user/powerpoint-prestacao-contas",
    ]
    logo_path = None
    for d in skill_dirs:
        candidato = os.path.join(d, "assets", "logo_service_white.png")
    ...
    if args.output:
        ...
        nome_clean = dados["condominio"].replace("Condomínio ", "").replace(" ", "")
        output = f"/home/claude/output/PrevisaoOrcamentaria2026_{nome_clean}.pptx"
