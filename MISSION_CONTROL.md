...
- **2026-07-07** — Previsão Orçamentária, Modo A: 3 gaps corrigidos + 1 hardening em `public/previsao-modulo.html`. Gap A (regex da linha de TAXA reconhece todos os rótulos, corrige reajuste zerado falso num condomínio real), Gap B camada 1 (total a ratear ancorado na linha de TAXA em vez de somado das categorias, corrige o total do rateio que saía inflado por somar o bloco de consumo, validado em condomínio real), Gap C (cor do texto ilegível na prévia da planilha) e hardening da Guarda 2 (bloqueia taxa base zero e linha de TAXA duplicada divergente). Gap B camada 2 concluído na mesma data (commit `943e342`): consumo excluído do panorama `cats` por comparação determinística de valor (taxa de fração ideal == "ordinárias" exclui, == "ordinárias + consumo" mantém), remove sempre a linha "Estimativa Boleto C/Variáveis (Média)" do panorama e tira o prefixo "CONTRATOS:" do nome da categoria. Harness Node em `scripts/previsao-harness/` valida `parsePlanilhaPronta` real do HTML contra 5 planilhas reais, agora 5/5 batendo inclusive o campo `cats`. Revisor e auditor de segurança aprovaram. Commits `a427f10..7d7cb7e` + `943e342`. Doc em `tarefas/concluidas/previsao-modo-a-gap-abc.md`. Pendências: validação visual no dev (aguarda push) e remoção de `skills-server/Previsao_Naturale_2026.xlsx` (planilha real de cliente no git).
